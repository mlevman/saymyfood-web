...
function lvl(){const s=S;s.p.x=200;s.e.x=760;if(!(s.hard&&s.level>0))s.p.hp=MAXHP;s.e.hp=MAXHP;s.p.apples=MAXA;s.e.apples=MAXA;s.p.charge=-1;s.e.charge=-1;s.apples=[];s.fx=[];s.lvT=0;s.lvEarned=0;s.pause=false;s.over=false;s.e.ai={t:0,target:null,want:0,thinkT:0}}
...
function award(n,px,py,col){S.total+=n;S.lvEarned=(S.lvEarned||0)+n;S.fx.push({x:px,y:py,t:0,kind:'score',n,col})}
...
if(Math.hypot(a.x-b.x,a.y-b.y)<22){a.dead=b.dead=.001;const mx=(a.x+b.x)/2,my=(a.y+b.y)/2;S.fx.push({x:mx,y:my,t:0,kind:'burst'});S.shake=Math.max(S.shake,.18);award(AW.clash,mx,my-14,'#F6E7A0');
...
S.apples=S.apples.filter(a=>a.dead<.4);S.fx=S.fx.filter(f=>{f.t+=dt;if(f.kind==='chunk'){f.vy+=G*dt;f.x+=f.vx*dt;f.y+=f.vy*dt}return f.t<(f.kind==='chunk'?.9:f.kind==='score'?1.1:.5)})}
...
x.lineWidth=4;x.strokeStyle='rgba(14,18,16,.85)';x.strokeText('+'+f.n,f.x,f.y-kk*46);x.fillStyle=f.col;x.fillText('+'+f.n,f.x,f.y-kk*46)}x.globalAlpha=1}
// charge bar
