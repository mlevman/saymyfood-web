...
// SITE-02 - owner will proofread: the new `notSaved`, `rotTap`, `paused` and `resume` keys in ru/es/pt below.
...
const txt={'t-back':'back',brd:'board','t-intro-h':'introH','t-intro-p':'introP','t-boy':'boy','t-boy-s':'boyS','t-girl':'girl','t-girl-s':'girlS',play:'play','t-how':'how','t-keys':'keys','p-gather':'pick','p-throw':'thr','t-time':'time','t-bonus':'bonus','t-lv':'lv',save:'save',again:'again','e-brd':'board','t-board':'board','b-close':'close','t-rot':'rot','t-rot-tap':'rotTap','t-pause-h':'paused','pz-resume':'resume','t-hard':'hard','t-hard-s':'hardS','t-round':'round','t-hearts':'heartsL','t-earned':'earned','t-all':'allT','rw-open':'rwOpen','t-rw-h':'rwH','rw-close':'close'};
...
// touch pad
document.querySelectorAll('#pad button').forEach(b=>{const k=b.dataset.k;const on=e=>{e.preventDefault();if(e.pointerType!=='mouse')markTouch();GAME.input[k]=true;b.classList.add('act')},off=e=>{e.preventDefault();GAME.input[k]=false;b.classList.remove('act')};
